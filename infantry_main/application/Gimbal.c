...
static void gimbal_init(){
   ...
   //下面请写PID角度环初始化
    ...
    gimbal.pitch.motor_measure->offset_ecd=7492;
...
static void gimbal_back_handle(){
    ...
    if(ABS(gimbal.relative_gyro_yaw)>2)
    {
        gimbal.yaw_is_back=0;//未回中
    }else
    {
        gimbal.yaw_is_back=1;//回中
    ...
    // pitch轴云台回中
    if(ABS(gimbal.pitch.relative_angle_get)>2){
        gimbal.pitch_is_back=0; //未回中
    }else{
        gimbal.pitch_is_back=1; //已回中
        gimbal.pitch.absolute_angle_set=0;//回中后绝对角度设为0
        gimbal.pitch.relative_angle_get=0;//相对角度设为0
    }
...
////  }
    ...
    if (!gimbal.yaw_is_back || !gimbal.pitch_is_back) {
        // pitch轴位置环控制
        gimbal.pitch.gyro_set = pid_loop_calc(&gimbal.pitch.angle_p,
    ...
    //动态
    gimbal.pitch.absolute_angle_set = fp32_constrain(gimbal.pitch.absolute_angle_set,
                                                     MIN_RELA_ANGLE + gimbal.pitch.absolute_angle_get - gimbal.pitch.relative_angle_get,
                                                     MAX_RELA_ANGLE + gimbal.pitch.absolute_angle_get - gimbal.pitch.relative_angle_get);
...
static void gimbal_relax_handle(){
    ...
    gimbal.trigger.give_current = 0; // 拨弹电机电流置零
...
static void gimbal_angle_update(){
    ...
    // pitch绝对角度和相对角度
    gimbal.pitch.absolute_angle_get = INS_angle[1] * MOTOR_RAD_TO_ANGLE; // 弧度转角度
    ...
    //yaw的绝对角度和相对角度，跟上面一样
    gimbal.yaw.absolute_angle_get = INS_angle[0] * MOTOR_RAD_TO_ANGLE; // 弧度转角度
    ...
    gyro_pitch = INS_gyro[1] * MOTOR_RAD_TO_ANGLE; // 弧度转角度
    gyro_yaw = INS_gyro[0] * MOTOR_RAD_TO_ANGLE;
    ...
    vision_data.roll = INS_angle[2] * MOTOR_RAD_TO_ANGLE;


    //把四元数存进去,四元数还是在INS中，提示：INS_quat[]
    vision_data.quaternion[0] = INS_quat[0];
    vision_data.quaternion[1] = INS_quat[1];
    vision_data.quaternion[2] = INS_quat[2];
    vision_data.quaternion[3] = INS_quat[3];
