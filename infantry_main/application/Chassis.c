...
static void chassis_init(chassis_t *chassis_ptr) {
    ...
    //扭动电机的pid初始化，在Chassic.h文件里可以看见一个chassis_vw_pid，是用来跟随云台的电机。云台
    pid_init(&chassis_ptr->chassis_vw_pid,CHASSIS_3508_PID_MAX_OUT,CHASSIS_3508_PID_MAX_IOUT,
        CHASSIS_3508_PID_KP,CHASSIS_3508_PID_KI,CHASSIS_3508_PID_KD);
    for(int i=0;i<4;i++)
    {
        pid_init(&chassis_ptr->motor_chassis[i].speed_p,CHASSIS_3508_PID_MAX_OUT,CHASSIS_3508_PID_MAX_IOUT,
            CHASSIS_3508_PID_KP,CHASSIS_3508_PID_KI,CHASSIS_3508_PID_KD);
        chassis_ptr->motor_chassis[i].motor_measure = motor_3508_measure + i;
    }

    //底盘驱动电机速度环初始化和电机数据结构体获取，这里是四个轮子的初始化，写个fof循环可以解
    ...
    chassis_ptr->last_mode=chassis_ptr->mode=CHASSIS_RELAX;
    chassis_ptr->spin_mode=NORMAL_SPIN;
...
static void chassis_set_mode(chassis_t* chassis){
    ...
    else if(robot_ctrl.target_lock)
    {
        chassis->last_mode=chassis->mode;
...
static void chassis_ctrl_info_get() {
    ...
    chassis.vx=(fp32)(rc_ctrl.rc.ch[CHASSIS_X_CHANNEL])*RC_TO_VX+chassis.vx_pc;
    chassis.vy=(fp32)(rc_ctrl.rc.ch[CHASSIS_Y_CHANNEL])*RC_TO_VY+chassis.vy_pc;
    chassis.vw=(fp32)(rc_ctrl.rc.ch[CHASSIS_Z_CHANNEL])*RC_TO_VW+chassis.vw_pc;

    VAL_LIMIT(chassis.vx,-(MAX_CHASSIS_VX_SPEED-0.300),(MAX_CHASSIS_VX_SPEED-0.300));
    VAL_LIMIT(chassis.vy,-(MAX_CHASSIS_VY_SPEED-0.300),(MAX_CHASSIS_VY_SPEED-0.300));
    VAL_LIMIT(chassis.vw,-(MAX_CHASSIS_VW_SPEED-0.300),(MAX_CHASSIS_VW_SPEED-0.300));
...
static void chassis_wheel_cal(){
    ...
    fp32 L=TREAD/100*sqrt(2);
    wheel_rpm[RF]=(vx+vy-vw*L)/WHEEL_MOTO_RATE;
    wheel_rpm[LF]=(-vx+vy-vw*L)/WHEEL_MOTO_RATE;
    wheel_rpm[LB]=(-vx-vy-vw*L)/WHEEL_MOTO_RATE;
    wheel_rpm[RB]=(vx-vy-vw*L)/WHEEL_MOTO_RATE;
    ...
    // for(int i=0;i<4;i++)
    // {
    //     chassis.motor_chassis[i].rpm_set=wheel_rpm[i];
    // }
}
...
static void chassis_wheel_loop_cal() {
    ...
    chassis.motor_chassis[RF].give_current=(int16_t)pid_calc(&chassis.motor_chassis[RF].speed_p,
        chassis.motor_chassis[RF].motor_measure->speed_rpm,chassis.motor_chassis[RF].rpm_set);
    ...
    chassis.motor_chassis[LF].give_current=(int16_t)pid_calc(&chassis.motor_chassis[LF].speed_p,
            chassis.motor_chassis[LF].motor_measure->speed_rpm,chassis.motor_chassis[LF].rpm_set);
    ...
    chassis.motor_chassis[RB].give_current=(int16_t)pid_calc(&chassis.motor_chassis[RB].speed_p,
            chassis.motor_chassis[RB].motor_measure->speed_rpm,chassis.motor_chassis[RB].rpm_set);
    ...
    chassis.motor_chassis[LB].give_current=(int16_t)pid_calc(&chassis.motor_chassis[LB].speed_p,
            chassis.motor_chassis[LB].motor_measure->speed_rpm,chassis.motor_chassis[LB].rpm_set);
/*
    for(int i=0;i<4;i++)
    {
        ramp_calc(&chassis_3508_ramp[i],chassis.motor_chassis[i].rpm_set);
        chassis_3508_ramp[i].out=pid_calc(&chassis.motor_chassis[i].speed_p,\
            chassis.motor_chassis[i].give_current,chassis.motor_chassis[i].rpm_set);
    }
*/
}
...
void chassis_can_send_back_mapping(){
    ...
    CAN_cmd_motor(CAN_1,CAN_MOTOR_0x200_ID,
        *real_motor_give_current[0],*real_motor_give_current[1],
        *real_motor_give_current[2],*real_motor_give_current[3]);
}
...
void chassis_follow_gimbal_handle(){
    ...
    //找到相对角度，根据这个相对角度旋转就行
    //1.得到云台的相对角度，转化为弧度制
    fp32 get_yaw_relative_rad=gimbal.yaw.relative_angle_get*ANGLE_TO_RAD;


    //2.算出这个弧度下的sin和cos是多少
    fp32 yaw_sin = sin(get_yaw_relative_rad);
    fp32 yaw_cos = cos(get_yaw_relative_rad);
    ...
    chassis.vx = vx_temp * yaw_cos + vy_temp * yaw_sin;
    chassis.vy = -vx_temp * yaw_sin + vy_temp * yaw_cos;


    //4.算出vw也就是角速度，使用pid算法，以云台相对角度为当前值，0作为设定值，算出角速度
    chassis.vw = pid_calc(&chassis.chassis_vw_pid,
                          gimbal.yaw.relative_angle_get, // 当前值
                          0);
    ...
    switch(chassis.spin_mode){
        ...
        case HIDDEN_ARMOR_SPEED_CHANGE:
            if((gimbal.yaw.relative_angle_get>=0 && gimbal.yaw.relative_angle_get<=60) ||
               ...
               //装甲板没有面向敌人的速度
                chassis.vw=MAX_CHASSIS_VW_SPEED*0.5f;
            }
            else{
                 //装甲板面向敌人的速度
                chassis.vw=MAX_CHASSIS_VW_SPEED*1.0f;
            }
    }
}
